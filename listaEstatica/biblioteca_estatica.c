...
printf("%s - %s - %d - %s - ", b->livros[i].titulo, b->livros[i].autor, b->livros[i].ano, b->livros[i].isbn);
if(b->livros[i].status == EMPRESTADO)
{
    printf("EMPRESTADO por %s no dia %s\n", b->livros[i].usuario,b->livros[i].dataEmprestimo);
}
else
    printf("DISPONIVEL\n");
}
